...
var datePickerController = (function datePickerController() {
    ...
    function parseUILanguage() {
        ...
        languageTag = !languageTag ? "ko" : languageTag.toLowerCase();
    ...
    function datePicker(options) {
        ...
        this.reposition = function() {
            ...
            o.div.style.left = Number(parseInt(trueBody.clientWidth+scrollLeft) < parseInt(osw+pos[0]) ? Math.abs(parseInt(((trueBody.clientWidth+scrollLeft) - osw)) - 120) : pos[0] - 120) + "px";
    ...
    return {
        ...
        // ID로 추가
        setDatePickerById:      function (targetId, dateFormat) {
			...
			if(targetId) {
				...
				paramObj.formElements = {};

				paramObj.formElements[targetId] = dateFormat;

			    this.createDatePicker(paramObj);
			}

			return document.getElementById(targetId);
        },

        // CLASS로 추가 (해당 클래스의 index번째 자식요소에 달력 추가- 0부터 시작)
        setDatePickerByClass:  function (classNm, index, dateFormat) {
        	...
        	if(classNm) {
        		...
        		var clzObj = document.getElementsByClassName(classNm);

        		for(var i = 0 ; i < clzObj.length ; i++) {

        			var upperObj  = clzObj[i];

        			var targetObj = upperObj.children[index];
        			var targetId  = targetObj.id;

        			if(!targetObj.disabled && targetObj.nodeName === "INPUT" && targetObj.type === "text") {

        				if(!targetId) {

        					var newId = "datePick_" + i;

        					targetObj.id = newId;
        					targetId     = newId;
        				}

        				this.destroyDatePicker(targetId);
        				this.setDatePickerById(targetId, dateFormat);
